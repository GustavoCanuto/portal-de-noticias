...
export default function saveFileLocal(pageContent,link,pagina,segundoLink,count) {
...
const fileName = path.join(folderPath,`${pagina} e ${segundoLink} e ${count}.html`);
let pageContentString = `<a href="${link}" target="_blank" >noticia original</a><br><br>` ;
 pageContentString += JSON.stringify(pageContent.conteudo, null, 2).replace(/\\/g, '');
fs.writeFileSync(fileName, pageContentString, 'utf8');
console.log('Arquivo salvo em html:', `${pagina} e ${segundoLink} e ${count}.html`);

const fileName2 = path.join(folderPath2,`${pagina} e ${segundoLink} e ${count}.txt`);
const pageContentString2 = JSON.stringify(pageContent, null, 2).replace(/\\/g, '');
fs.writeFileSync(fileName2, pageContentString2, 'utf8');
 console.log('Arquivo salvo em texto:', `${pagina} e ${segundoLink} e ${count}.txt`);
